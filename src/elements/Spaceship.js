...
class Spaceship {
  ...
  constructor() {
    ...
    this.element = kbm.add([
      ...
      kbm.pos(
        constants.width / 2,
        constants.height -
          resources.spaceship.height * utils.responsiveFactor(),
      ),
      kbm.anchor('center'),
    ...
    this.rightBound =
      constants.width -
      (resources.spaceship.width * utils.responsiveFactor()) / 2;
    this.topBound = (resources.spaceship.height * utils.responsiveFactor()) / 2;
    this.bottomBound =
      constants.height -
      (resources.spaceship.height * utils.responsiveFactor()) / 2;
  }
  ...
  registerControlsForTouchscreen() {
    kbm.onMouseDown(() => {
      ...
      const spaceshipDirection =
        mouseXCoord < constants.width / 2 ? 'LEFT' : 'RIGHT';
  ...
  explode() {
    ...
    const timer = setInterval(() => {
      if (explosionImageCount == 1)
        this.element.scaleTo(
          this.element.scale.x + 2,
          this.element.scale.y + 2,
        );
  ...
  decreaseHealthStatus() {
    this.collisions++;
    const healthStatusCategoriesCount = Object.keys(
      Spaceship.HEALTH_STATUS,
    ).length;
    const nOfcollisionPerStatus =
      constants.spaceshipCollisionCapacity / healthStatusCategoriesCount;
